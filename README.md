...
Библиотека декоративных слоев находится в:

```text
static/decor/decor_library.json
```

Изображения хранятся по категориям:

```text
static/decor/{category}/
```

На первом этапе элементы каталога могут иметь `enabled=false`, пока реальные PNG/WebP-файлы не добавлены.

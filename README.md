...
Сервис умеет генерировать до 5 фоновых изображений через OpenAI Image API.

Изображения:

- не используются на первом и последнем слайде;
- применяются только к внутренним слайдам;
- сохраняются в `static/generated/{project_id}/`;
- встраиваются как фоновые визуальные акценты с сохранением читаемости контента.
